    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-3.5 shadow-sm max-w-[280px] text-left">
      ...
      <div className="space-y-3 max-h-[180px] overflow-y-auto pr-1 custom-scrollbar">
        {checkoutItems.map((item, index) => (
          <div key={index} className="flex gap-2.5 items-start pb-2.5 border-b border-gray-50 last:border-0 last:pb-0">
            ...
            <div className="flex flex-col min-w-0 flex-1">
              <span className="text-xs font-semibold text-gray-900 line-clamp-1 leading-tight">{item.title}</span>
              <span className="text-[10px] text-gray-400 mt-0.5">Phân loại: {item.colorSelected || "Mặc định"}</span>
              <div className="flex justify-between items-center mt-1">
                <span className="text-[11px] text-gray-500 font-medium">Số lượng: {item.quantity}</span>
                <span className="text-xs font-bold text-gray-950">{formatPrice(item.price)}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="mt-3 pt-2.5 border-t border-dashed border-gray-200 flex justify-between items-center">
        ...
        <span className="text-sm font-black text-[#e30019]">{formatPrice(totalAmount)}</span>
      </div>
    </div>
  );
};
