...
const AdminChat = () => {
  // --- CÁC STATE QUẢN LÝ ---
  const [activeUserId, setActiveUserId] = useState(mockUsers?.[0]?.userId || null);
  const [inputText, setInputText] = useState("");
  const [searchQuery, setSearchQuery] = useState(""); // LỖI 4: State phục vụ tính năng tìm kiếm khách hàng
  const messagesEndRef = useRef(null);

  // --- LỖI 2: HÀM TỰ ĐỘNG LẤY TIN NHẮN CUỐI CÙNG CỦA ĐOẠN CHAT ---
  const getLastMessageDetails = (userId) => {
    const userMsgs = mockMessages?.filter(
      (msg) => msg.senderId === userId || msg.receiverId === userId
    ) || [];

    if (userMsgs.length === 0) {
      return { content: "Chưa có cuộc hội thoại", time: "" };
    }

    // Sắp xếp tin nhắn theo thời gian tăng dần và lấy tin nhắn cuối cùng
    const sortedMsgs = [...userMsgs].sort((a, b) => a.timestamp - b.timestamp);
    const lastMsg = sortedMsgs[sortedMsgs.length - 1];

    // Định dạng giờ hiển thị cho tin nhắn cuối
    const timeString = new Date(lastMsg.timestamp).toLocaleTimeString("vi-VN", {
      hour: "2-digit",
      minute: "2-digit",
    });

    if (lastMsg.type === "order") {
      return { content: lastMsg.content ? `[Đơn hàng] ${lastMsg.content}` : "[Gửi một đơn hàng]", time: timeString };
    }
    return { content: lastMsg.content, time: timeString };
  };
  ...
  const activeUser = (mockUsers || []).find((u) => u.userId === activeUserId);

  // Tự động cuộn xuống cuối khi có tin nhắn mới hoặc đổi người chat
  useEffect(() => {
    messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [activeUserId, mockMessages]);

  const handleSendMessage = (e) => {
    e.preventDefault();
    if (!inputText.trim()) return;
    console.log("Admin gửi tin nhắn:", inputText);
    setInputText("");
  };
        ...
        <div className="flex-1 overflow-y-auto">
          {filteredUsers.length > 0 ? (
            filteredUsers.map((user) => {
              ...
              const lastMsgDetails = getLastMessageDetails(user.userId);
                ...
                <div key={msg.messageId} className={`flex ${isAdmin ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[75%] ${isAdmin ? "items-end" : "items-start"} flex flex-col gap-1`}>
                    ...
                    {msg.type === "order" && msg.orderData && (
                      <div className={`mt-1 bg-white border border-gray-200 p-4 rounded-2xl shadow-sm w-[320px] ${isAdmin ? "rounded-br-sm" : "rounded-bl-sm"}`}>
                        <div className="flex items-center gap-2 mb-3 pb-3 border-b border-gray-100">
                          ...
                          <span className="font-bold text-gray-800 text-sm">Thông tin đơn hàng</span>
                        </div>
                        
                        <div className="space-y-3">
                          {msg.orderData.checkoutItems?.map((item, idx) => (
                            <div key={idx} className="flex gap-3 items-center">
                              <img src={item.thumbnail} alt={item.title} className="w-12 h-12 rounded-lg border border-gray-100 object-contain p-1 flex-shrink-0 bg-white" />
                              <div className="flex-1 min-w-0">
                                <h4 className="text-xs font-bold text-gray-800 truncate">{item.title}</h4>
                                <p className="text-[11px] text-gray-500">{item.colorSelected} x {item.quantity}</p>
                              </div>
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-100 flex justify-between items-center">
                          ...
                          <span className="text-sm font-black text-red-500">
                            {msg.orderData.totalAmount?.toLocaleString("vi-VN")} ₫
                          </span>
                        </div>
                      </div>
                    )}
