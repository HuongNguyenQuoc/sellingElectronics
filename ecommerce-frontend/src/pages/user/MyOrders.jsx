...
const MyOrders = () => {
  ...
  const getProductDetails = (productId) => {
    ...
    if (!product) return { title: "Sản phẩm không tồn tại", price: 0, thumbnail: "https://placehold.co/100x100?text=No+Image" };
    return {
      title: product.title,
      price: product.price,
  ...
  const confirmCancelOrder = () => {
    console.log(`Đã xác nhận hủy đơn hàng: ${cancelModal.orderId}`);
        ...
        <div className="space-y-6">
          {filteredOrders.length > 0 ? (
            filteredOrders.map((order) => {
              ...
              return (
                <div key={order.orderId} className="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <div className="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-50/50 gap-4">
                    <div className="flex items-center gap-4">
                      <span className="font-semibold text-gray-900 tracking-wider">#{order.orderId}</span>
                  ...
                  <div className="px-6 py-5 bg-gray-50/50 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="w-full sm:w-auto flex items-center justify-between sm:justify-start gap-4">
                      ...
                      <span className="text-xl font-bold text-[#e30019]">{order.totalAmount.toLocaleString("vi-VN")} ₫</span>
                    </div>
              ...
              <div className="flex gap-4 items-start">
                ...
                <div className="mt-1">
                  ...
                  <p className="text-sm text-gray-500 leading-relaxed">
                    Bạn có chắc chắn muốn hủy đơn hàng <span className="font-bold text-gray-800">#{cancelModal.orderId}</span> không? Hành động này sẽ không thể hoàn tác.
                  </p>
                </div>
              </div>
            </div>
