...
const PhonesPage = () => {
  const { products: allPhones, isLoading, error } = useProducts("smartphone");
  const [selectedBrands, setSelectedBrands] = useState([]);
  const [selectedPrices, setSelectedPrices] = useState([]);
  const [sortOrder, setSortOrder] = useState("asc");

  const availableBrands = useMemo(() => getUniqueBrands(allPhones), [allPhones]);
  ...
  const filteredPhones = useMemo(() => {
    const filtered = allPhones.filter((phone) => {
      const matchBrand =
        selectedBrands.length === 0 || selectedBrands.includes(phone.brandName);
      const matchPrice =
        selectedPrices.length === 0 ||
        selectedPrices.some((priceId) => {
          const range = priceRanges.find((item) => item.id === priceId);
          return (phone.price || 0) >= range.min && (phone.price || 0) <= range.max;
        });
      ...
      <div className="bg-gray-900 text-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 relative z-20">
          ...
          <p className="text-gray-300 max-w-xl text-lg">
            Khám phá các thiết bị di động với công nghệ tiên tiến nhất. Định hình lại giới hạn của một chiếc smartphone.
          </p>
        </div>
      </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filteredPhones.map((phone) => {
                const productId = getProductId(phone);
